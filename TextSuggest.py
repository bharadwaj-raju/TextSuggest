    ...
    if args.word:
        ...
        if args.autosel:
            ...
            if args.autosel == 'beginning':
                ...
                # Ctrl + Shift + ->
                sp.Popen(
                    ['xdotool keydown Ctrl keydown Shift key Right keyup Shift keyup Ctrl > /dev/null'], shell=True)

            elif args.autosel == 'middle':

                # Ctrl + <- then Ctrl + Shift + ->
                sp.Popen(
                    ['sleep 0.5; xdotool key Ctrl+Left; xdotool key Ctrl+Shift+Right > /dev/null'], shell=True)

            else:

                # Ctrl + Shift + <-
                sp.Popen(
                    ['sleep 0.5; xdotool key Ctrl+Shift+Left > /dev/null'], shell=True)
...
custom_words_file = os.path.expanduser(
...
def type_command_output(command):
    ...
    if '\n' in command_out:
        ...
        for i in command_out_newl_list:
            ...
            sp.Popen(['xdotool type --clearmodifiers "%s"; xdotool keydown Shift key Return keyup Shift' %
                      i.strip('\'').strip('"')], shell=True)
...
def get_suggestions(suggestion_string):
    ...
    suggestions = sorted(suggestions, key=Counter(
        suggestions).get, reverse=True)
...
def display_dialog_list(item_list):
    ...
    mouse_loc_raw = sp.check_output(
        ['xdotool getmouselocation --shell'], shell=True)
    ...
    if args.font:
        ...
        # Font should be specified in Pango format: FontName {(optional)
        # FontWeight} FontSize
    ...
    popup_menu_cmd_str = 'echo "%s" | rofi -dmenu -fuzzy -p "> " -i %s -font "%s" -xoffset %s -yoffset %s -location 1' % (
        items_string, rofi_theme, font, x, y)
    ...
    if suggestion is None or suggestion == b'' or suggestion == []:
        ...
        if '=' in suggestion:
            ...
            if '#' in expand_suggestion:
                ...
                sp.Popen(['xdotool type \'%s\'' %
                          expand_suggestion.rstrip()], shell=True)
